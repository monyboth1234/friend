...
<body>
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li class="nav-item my-1">
                    ...
                    <ul class="list-unstyled ps-2 mb-0">
                        ...
                        <li>
                            <a href="{{ route('Farm_Animals') }}" class="nav-link sub-nav-link d-flex align-items-center {{ request()->routeIs('Farm_Animals*') ? 'active' : '' }}">
    ...
    <main class="main-content">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 pb-3 border-bottom bg-white p-3 rounded-3 shadow-sm">
            <div>
                <h3 class="fw-bold mb-1 text-success">សត្វផ្សាយ(Farm Animals)</h3>
                <span class="text-muted"><i class="bi bi-envelope me-1"></i> អ៊ីមែល៖ {{ Auth::user()->email }}</span>
            </div>
            <button class="btn btn-success d-flex align-items-center gap-2 shadow-sm px-3 py-2" data-bs-toggle="modal" data-bs-target="#addFarmAnimalModal">
                ...
                <div class="card border-0 shadow-sm rounded-3 stat-card h-100">
                    <div class="card-body d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h4 class="card-title fw-bold mb-0 text-success">{{ number_format($totalAnimal) }}</h4>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-sm-6 col-xl-4">
                <div class="card border-0 shadow-sm rounded-3 stat-card h-100">
                    <div class="card-body d-flex align-items-center">
                        ...
                        <div>
                            <h6 class="card-subtitle text-muted mb-1">ចំនួនប្រភេទសត្វ</h6>
                            <h4 class="card-title fw-bold mb-0 text-success">{{ $farmAnimals->count() }}</h4>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($farmAnimals as $index => $farmAnimal)
                                <tr>
                                    <td class="text-center fw-bold">{{ $index + 1 }}</td>
                                    <td>
                                        @if($farmAnimal->image)
                                            <img src="{{ $farmAnimal->image }}" alt="{{ $farmAnimal->name }}" width="60" height="60" class="rounded-3 shadow-sm" style="object-fit: cover;">
                                        @else
                                    ...
                                    <td>
                                        <span class="fw-semibold text-success">{{ $farmAnimal->name }}</span>
                                    </td>
                                    <td class="text-center">
                                        <span class="badge bg-success-subtle text-success fs-6">${{ number_format($farmAnimal->price, 2) }}</span>
                                    </td>
                                    <td class="text-center">
                                        @if($farmAnimal->qty > 0)
                                            <span class="badge bg-primary-subtle text-primary">{{ number_format($farmAnimal->qty) }}</span>
                                        @else
                                            <span class="badge bg-danger">អស់ពីស្តុក</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($farmAnimal->description)
                                            <span class="text-muted">{{ Str::limit($farmAnimal->description, 50) }}</span>
                                        @else
                                            <span class="text-muted fst-italic">មិនមាន</span>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        <small class="text-muted">{{ $farmAnimal->created_at->format('d/m/Y') }}</small>
                                    </td>
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            <button type="button" 
                                                    ...
                                                    data-bs-target="#editFarmAnimalModal{{ $farmAnimal->id }}" 
                                                    title="កែប្រែ">
                                                <i class="bi bi-pencil-square"></i>
                                            </button>

                                            {{-- Delete Form --}}
                                            <form action="{{ route('farm_animals.destroy', $farmAnimal->id) }}" method="POST" onsubmit="return confirm('តើអ្នកពិតជាចង់លុបសត្វនេះមែនទេ?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                {{-- Edit Farm Animal Modal (បំបែកសម្រាប់ item នីមួយៗ) --}}
                                <div class="modal fade" id="editFarmAnimalModal{{ $farmAnimal->id }}" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-lg modal-dialog-centered">
                                        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
                                            <div class="modal-header bg-warning text-dark">
                                                <h5 class="modal-title fw-bold">
                                                    <i class="bi bi-pencil-square me-2"></i>                                                     កែប្រែព័ត៌មានសត្វ
                                                </h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body p-4">
                                                <form action="{{ route('farm_animals.update', $farmAnimal->id) }}" method="POST" enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')

                                                    <div class="mb-3 text-start">
                                                        <label class="form-label fw-semibold">ឈ្មោះសត្វ <span class="text-danger">*</span></label>
                                                        <input type="text" name="name" class="form-control" value="{{ old('name', $farmAnimal->name) }}" required>
                                                    </div>
                                                    ...
                                                    <div class="row text-start">
                                                        <div class="col-md-6 mb-3">
                                                            ...
                                                            <div class="input-group">
                                                                ...
                                                                <input type="number" name="price" class="form-control" step="0.01" min="0" value="{{ old('price', $farmAnimal->price) }}" required>
                                                            </div>
                                                        </div>

                                                        <div class="col-md-6 mb-3">
                                                            <label class="form-label fw-semibold">ចំនួន <span class="text-danger">*</span></label>
                                                                <input type="number" name="qty" class="form-control" min="0" value="{{ old('qty', $farmAnimal->qty) }}" required>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="mb-3 text-start">
                                                        ...
                                                        @if($farmAnimal->image)
                                                            <div class="mb-2">
                                                                <img src="{{ $farmAnimal->image }}" alt="{{ $farmAnimal->name }}" width="70" class="rounded border shadow-sm">
                                                            </div>
                                                        @endif
                                                    ...
                                                    <div class="mb-4 text-start">
                                                        ...
                                                        <textarea name="description" class="form-control" rows="3">{{ old('description', $farmAnimal->description) }}</textarea>
                                                    </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-5">
                                        ...
                                        <h6 class="text-muted mt-3">មិនទាន់មានសត្វទេ</h6>
                                        <button class="btn btn-success btn-sm mt-2" data-bs-toggle="modal" data-bs-target="#addFarmAnimalModal">
    ...
    <div class="modal fade" id="addFarmAnimalModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title fw-bold">
                        <i class="bi bi-bug me-2"></i> បន្ថែមសត្វថ្មី
                    </h5>
                ...
                <div class="modal-body p-4">
                    <form action="{{ route('farm_animals.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">ឈ្មោះសត្វ <span class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control" placeholder="ឧ. Cow" value="{{ old('name') }}" required>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">តម្លៃ <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="number" name="price" class="form-control" step="0.01" min="0" placeholder="ឧ. 200.00" value="{{ old('price') }}" required>
                                </div>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">ចំនួន <span class="text-danger">*</span></label>
